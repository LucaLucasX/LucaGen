...
# 在文件顶部加入
from tools.CloudswaySearchClient import CloudswaySearchClient
...
class SearchTool(Tool):
    ...
    1. 混合模式 (hybrid) - 智能选择TAVILY或SERPAPI或者Cloudsway
    2. Tavily API (tavily) - 专业AI搜索
    3. SerpApi (serpapi) - 传统Google搜索
    """

    def __init__(self, backend: str = "hybrid", tavily_key: Optional[str] = None, serpapi_key: Optional[str] = None, cloudsway_key: Optional[str] = None):
        super().__init__(
            name="search",
            description="一个智能网页搜索引擎。支持混合搜索模式，自动选择最佳搜索源。当你需要回答关于时事、事实以及在你的知识库中找不到的信息时，应使用此工具。"
        )
        self.backend = backend
        self.cloudsway_key = cloudsway_key or os.getenv("CLOUDSWAY_API_KEY")
    ...
    def _setup_backends(self):
        ...
        # 检查Cloudsway可用性
        if self.cloudsway_key:
            try:
                self.cloudsway_client = CloudswaySearchClient(api_key=self.cloudsway_key)
                self.available_backends.append("cloudsway")
                print("✅ Cloudsway 搜索引擎已初始化")
            except Exception as e:
                print(f"⚠️ Cloudsway 初始化失败: {e}")
        else:
            print("⚠️ CLOUDSWAY_API_KEY未设置")
        ...
        if "tavily" in self.available_backends:
            try:
                ...
                if "serpapi" in self.available_backends:
                    print("🔄 切换到SerpApi搜索")
                    return self._search_serpapi(query)
        # 其次 Cloudsway（强网页抓取 + 全文爬取）
        elif "cloudsway" in self.available_backends:
            try:
                print("🎯 使用 Cloudsway 进行全文智能搜索")
                return self._search_cloudsway(query)
            except Exception as e:
                print(f"⚠️ Cloudsway 搜索失败: {e}")
    ...
    def _search_cloudsway(self, query: str) -> str:
        if not self.cloudsway_client:
            return "错误：Cloudsway 未初始化"

        data = self.cloudsway_client.search(query, count=5)

        result = "🔍 Cloudsway 全文搜索结果：\n\n"
