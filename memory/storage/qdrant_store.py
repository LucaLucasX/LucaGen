...
class QdrantVectorStore:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            if self.url and self.api_key:
            ...
            elif "localhost" in self.url.lower():
                # 使用本地Docker服务（默认）
                self.client = QdrantClient(
                    host="localhost",
                    port=6333,
                ...
                logger.info("✅ 成功连接到本地Docker Qdrant服务: localhost:6333")
            else:
                # 使用本地服务（默认）
                local_path = os.getenv("QDRANT_LOCAL_PATH", "./memory_data/qdrant_local_store")
                try:
                    self.client = QdrantClient(path=local_path, timeout=self.timeout)
                    logger.info(f"✅ 成功连接到本地Qdrant服务: {local_path}")
                except RuntimeError as e:
                    # 捕获本地锁报错，回退ChromaDB
                    if "already accessed" in str(e):
                        logger.info(f"⚠️ 本地Qdrant路径 {local_path} 已被占用，回退ChromaDB存储")
                        from memory.storage.chroma_store import ChromaVectorStore
                        self.client = ChromaVectorStore(
                            collection_name=self.collection_name,
                            persist_path="./memory_data/chroma_store",
                            vector_size = self.vector_size
                        )
                        logger.info("✅ 使用ChromaDB代替本地Qdrant服务")
                    else:
                        raise

                # 如果是Qdrant client，继续创建集合
            if isinstance(self.client, QdrantClient):
