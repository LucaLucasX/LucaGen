...
class OpenAIEmbedding(EmbeddingModel):
    """
    标准 OpenAI Embedding 模型
    - 使用官方 SDK：client.embeddings.create()
    """
    ...
    def __init__(self, model_name: str = "text-embedding-3-small", api_key: Optional[str] = None, base_url: Optional[str] = None):
        ...
        self.base_url = base_url or os.getenv("EMBED_BASE_URL")
        if not self.api_key:
    ...
    def encode(self, texts: Union[str, List[str]]):
        ...
        # resp.data[i].embedding 是 list[float]
...
def _build_embedder() -> EmbeddingModel:
    preferred = os.getenv("EMBED_MODEL_TYPE", "OpenAI").strip()
    # 根据提供商选择默认模型
    ...
    if api_key and preferred != "local":
        kwargs["api_key"] = api_key
    base_url = os.getenv("EMBED_BASE_URL")
    if base_url and preferred != "local":
